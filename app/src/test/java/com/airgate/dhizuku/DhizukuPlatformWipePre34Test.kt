...
/**
 * Robolectric verification of the pre-34 device wipe path: the literal
 * `DevicePolicyManager.wipeData(flags)` invocation is exercised against a
 ...
 * branch asserts that the platform `wipeData` call — the only option below API
 * 34 — was the invocation made.
 */
@RunWith(AndroidJUnit4::class)
@Config(shadows = [ShadowDevicePolicyManagerForWipe::class], sdk = [33])
class DhizukuPlatformWipePre34Test {
    ...
    @Test
    fun `a successful wipeData platform call is reported as ACCEPTED`() {
        ...
        assertEquals(WipeCall.WIPE_DATA, shadow.lastWipeCall)
    }

    @Test
    fun `a throwing wipeData platform call is reported as REJECTED`() {
