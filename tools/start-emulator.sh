...
#
# The emulator is booted headless with snapshots disabled: the windowed,
# snapshot-booted emulator is unstable on this host (crashes shortly after
# loading 'default_boot'). Screenshot capture uses the emulator console
# (`adb emu screenrecord screenshot`), which needs a live emulator.
#
...
echo "start-emulator: no device connected — booting AVD '$AVD' headless"
nohup "$EMU" -avd "$AVD" -no-snapshot -no-window -no-audio -no-boot-anim -gpu swiftshader_indirect >/dev/null 2>&1 &
disown || true
