...
- **COUNTDOWN WIPE / WIPING** — the threshold was reached and the wipe path is running (see step 8). A countdown appears only when a precise wipe deadline can be armed; if exact-alarm access is unavailable, the path fails closed to an immediate wipe.

**The Protection switch.** The "Protection" card is the master on/off for the background watchdog. The app deliberately starts **paused**, so you can review the settings before anything is armed — an alarm must never fire before you have seen what is installed. Tap the switch to arm the watchdog when you are ready. The switch will only arm once a usable Armed PIN is set, the app can post notifications, Bluetooth detection is allowed (BLUETOOTH_CONNECT on Android 12+), and **Exact alarms** access is granted ("Alarms & reminders" on Android 12+) — arming a device whose alarm path could be entirely silent, whose Bluetooth detection is blind, or whose precise wipe deadline could never fire on time is refused (you'll be told which one is missing).
...
- **Exact alarms** — the precise wipe countdown is armed as an exact alarm; without the "Alarms & reminders" access (Android 12+) that alarm could never fire on time. Without it, arming is refused. Tap "Allow Exact Alarms" to open the system setting and grant it.
...
- **Pre-Wipe Grace Window** (0 s, 0–3600 s) — if greater than zero, reaching the threshold starts a **countdown** instead of wiping instantly, giving you a chance to disarm. The countdown runs as an exact alarm, so it is only armed while the **Exact alarms** access is granted; if that access is ever unavailable, the path fails closed to an immediate wipe. You'd use a grace window if you want a last-moment escape for false positives, at the cost of giving a genuinely compromised phone extra time.
...
**The countdown.** When a grace window is configured and the exact-alarm access is granted, the wipe is delayed behind a countdown you can cancel with your Armed PIN. The deadline is a precise, exact alarm on a monotonic clock — a clock change cannot stretch it. If that exact alarm can no longer be armed (for example because "Alarms & reminders" access was revoked, which deletes every exact alarm and restarts the app), the app fails closed: instead of presenting a deadline it can no longer guarantee, it executes the wipe immediately, and the in-app alarm is recorded distinctly as **"WIPE EXECUTED — EXACT ALARM LOST"** (or **"WIPE FAILED — EXACT ALARM LOST"** if the system refuses the wipe) so this fail-closed escalation is never mistaken for a normal wipe.

**The wipe screen.** If the wipe executes, the app shows the emergency screen. In **Dry-Run mode** (the default) it is clearly marked **"SIMULATION — NO DATA WILL BE DESTROYED"**: a rehearsal of the wipe, no data lost. Recovering from a simulated wipe requires your Armed PIN, which resets the threat score and returns you to the dashboard. With **Dry-Run off**, the same screen shows **"LIVE WIPE — THIS DEVICE WILL BE FACTORY-RESET"** and precedes a **real, non-recoverable** factory reset. There is no undo — that is the point: an adversary who coerces you into unlocking gets a brick, not a wallet.
